...
from app.handlers.base_handler import BaseChatHandler
...
from app.helpers.tenacity_retry_strategies import openai_tools_calling_retry_strategy
...
from app.models.gpt_function_output_model import GPTFunctionOutput
...
from app.redis_services.redis_methods import get_assistant_part_id
...
class OutOfScopeChatHandler(BaseChatHandler):
    ...
    async def handle(self, message: ChatMessage) -> HandlerResponse:
        ...
        return HandlerResponse(
            status=HandlerResponseStatus.SUCCESS, message=gpt_response_message
        )
    ...
    async def process_gpt_response(self) -> Union[str, None]:
        ...
        self.log_message(
            f"GPT failed to provide a string response {MAX_LOOP_COUNT} times in a row"
        )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"GPT Failed to respond {MAX_LOOP_COUNT} times",
        )
    ...
    async def process_tool_calls(
        ...
        tool_call_coroutines = [
            self.handle_tool_call(tool_call) for tool_call in tool_calls
        ]
        function_results = await asyncio.gather(
            *tool_call_coroutines, return_exceptions=True
        )
    ...
    async def handle_tool_call(self, tool_call) -> RedisMessages:
        ...
        try:
            self.log_message(
                f"Calling {gpt_function_name} function with {gpt_arguments} arguments."
            )
            function_response: GPTFunctionOutput = await function_to_call(
                default_function_arguments, **gpt_arguments
            )

            self.log_message(
                f"Function {gpt_function_name} response: {function_response.to_dict()}"
            )
