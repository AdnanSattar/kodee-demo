...
from app.database.database_models.events_table_model import EventsTable
from app.database.database_models.history_table_model import HistoryTable
from app.helpers.tenacity_retry_strategies import postgresql_retry_strategy
from app.utils.env_constants import DB_HOST, DB_NAME, DB_PASSWORD, DB_USERNAME
...
class AsyncPostgreSQLDatabase:
    ...
    async def connect(
        self, connection_timeout=ACQUIRE_A_CONNECTION_MAX_TIMEOUT
    ) -> None:
        if not self.pool:
    ...
    async def _fetch_query(
        self, query, *args, command_timeout=COMMAND_MAX_EXECUTION_TIMEOUT
    ):
        try:
            ...
            async with self.pool.acquire(timeout=command_timeout) as connection:
                async with connection.transaction():
                    return await connection.execute(
                        query, *args, timeout=command_timeout
                    )
        except TimeoutError as e:
            logger.log(
