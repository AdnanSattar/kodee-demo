...
from app.helpers.tenacity_retry_strategies import chatbot_label_retry_strategy
...
from app.router.gpt_router_prompts import get_router_prompt
...
async def generate_chatbot_label(conversation_id: str, user_id: str) -> ChatbotLabel:
    ...
    messages = await get_conversation_history_with_system_prompt(
        system_description, conversation_id
    )
    ...
    if not gpt_response:
        logger.log(
            "GPT failed to generate a response.", conversation_id=conversation_id
        )
    ...
    if not await is_chatbot_label_valid(decoded_chatbot_label):
        logger.log(
            "GPT response was not a valid chatbot label.",
